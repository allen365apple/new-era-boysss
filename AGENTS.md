...
4. **`部落格改寫規則.md`** — 「逐字稿 → 文章」的唯一正本（目前 v4.0）。做內容生產線一定要看。
...
> 想查「某條規則是什麼時候、為什麼改的」→ `docs/規則沿革.md`（不必每次讀）。
>
...
- 寫作規則：`部落格改寫規則.md` v4.0；標題為 SEO／AEO 從逐字稿關鍵字改寫、採「關鍵字優先型」（核心概念詞放句首、≤約30全形字、上下集差異化）、開頭＝本集簡介（綜合 RSS＋逐字稿自己寫、不照抄）、先確定一條文章主線並讓 4–5 節按起承轉合推進、每節 2 個正文段落、金句署名放進引用區塊內、文末分隔線 + 2 段收尾且融入一個性別理論概念（自然帶出、不掛學術招牌，專有名詞用「」標出並解釋一句）。
...
| Podcast 人名、來賓、作品名與自創詞 | `docs/KB2_人名與專有名詞對照表.md` |
...
逐日的決策與 commit 流水帳搬到 **`docs/規則沿革.md`**（2026-09-11 搬移，內容一字未改）。
要查「某條規則什麼時候改的、被哪一版取代」就去那份；日常開工只需要看上面的「目前狀態快照」與 `docs/HANDOFF.md`。

### 維護這份紀錄的規則

1. 完成一批會影響後續工作的變更後，在 `docs/規則沿革.md` 最上方按日期新增摘要與 commit；若同時改變現況，回頭更新本檔的「目前狀態快照」。
2. 詳細規則只改對應正本；本節只記「改了什麼、正本在哪裡、是否已發布」。
3. 若新決策推翻舊決策，更新「目前狀態快照」，並在沿革中明寫「取代哪一版」，不要讓下一位 AI 猜。
...
| `docs/KB2_人名與專有名詞對照表.md` | Podcast 產線共用人名、來賓、作品名與自創詞對照表；不是可跳過的參考資料。 |
...
| `docs/規則沿革.md` | 規則與決策的歷史紀錄（原本在 AGENTS.md 的「近期更新」）；要查某條規則何時改、被哪版取代時看。 |
