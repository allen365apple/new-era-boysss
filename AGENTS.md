...
4. **`部落格改寫規則.md`** — 「逐字稿 → 文章」的唯一正本（目前 v3.7）。做內容生產線一定要看。
...
- 線上 `main` 功能基線：`6ef29ec`。網站由 GitHub `main` 觸發 Cloudflare Pages 自動部署。
- 已公開文章：EP1、EP2、EP3、EP4、EP5、EP48、EP49、EP64。
- 寫作規則：`部落格改寫規則.md` v3.7；4–5 節、每節至少兩個正文段落、穿插逐字稿引言、文末分隔線與 1–2 段小結。
- 去 AI 味：已把 `speak-human-tw` 納入文章第二輪校稿；自動產線採「跳過確認、事後摘要」，但集數、日期、名稱、公開暱稱、平台連結與已選定引言屬保護內容。
...
- 行動版：議題索引與單一議題頁已修正橫向溢出；390px 裝置實測內容可完整落在視窗內。
