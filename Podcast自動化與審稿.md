...
5. 依 `部落格改寫規則.md` 產生含 `---` 分隔線、文末小結、`draft: false` 的 Markdown 文章；前後收聽文案與連結由版型自動顯示。
6. 使用 `speak-human-tw` 的部落格「中」力度做第二輪校稿。自動產線採「跳過確認、事後摘要」，先保護集數、日期、專有名詞、公開暱稱、平台連結與已選定引言，再檢查 38 種 AI 痕跡、台灣用語與全形標點；摘要只留在任務紀錄，不寫進文章。
